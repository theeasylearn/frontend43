...
app.use(function (request, response, next) {
    ...
    if (hours < 10 || hours > 18)
        response.send('our library works only between 10am to 6pm');
    next();

});
//2nd middleware
app.use(function (request, response, next) {
    console.log('i am 2nd middleware function...');
    let currentTime = new Date();
    let datetime = currentTime.getHours() + ":" + currentTime.getMinutes() + ":" + currentTime.getSeconds() + " " + currentTime.getDate() + "/" + (currentTime.getMonth()+ 1) + "/" + currentTime.getFullYear();
    let content = `Request url ${request.url} Method ${request.method} Request Time = ${datetime} \n`;
